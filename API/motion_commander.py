...
class MotionCommander:
    ...
    def __init__(self, crazyflie, default_height=0.3,isflying=False,position=[0,0,0]):
        logger.info(f"Motion_commander starting")
        ...
        self._is_flying = isflying
        if(self._is_flying):
            self.up(default_height-position[2], VELOCITY)
            logger.info(f"Motion_commander is flying : {self._is_flying}")
        self._thread = None
