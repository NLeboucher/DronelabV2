...
class SwarmControl:
    ...
    def __init__(self, default_height=0.3, default_z_speed=0.2):
        global SWARM, factory, DRONES, commander, DefaultZSpeed, DefaultHeight, positions, i
        self.DRONES = []
        # SWARM = None
        # factory = None
    ...
    def OpenLinks(self,):
        ...
        self.CloseLinks()
        global SWARM, URIS, factory, DRONES, URIScopy
        URIS = URIScopy.copy()
        logger.info(f"URIS: {URIS}, URIScopy{URIScopy}")
        SWARM = Swarm(URIS, factory)
        self.DRONES = []
        logger.info(f"{URIS}")
        ...
        if(not SWARM._is_open):
            SwarmControl.CloseLinks()

        
        return OutputDict(self.DRONES,"URIS").dict
    def SetHomes(self,):
        SwarmControl.All_GetEstimatedPositions()
        for i in range(0, len(URIS)):
            self.id = i.copy()
            logger.info(f"Setting Home for {self.DRONES[i]} at {self.positions[self.DRONES[i]]} to check if it is {self.homes[i]}")
            self.homes = positions.copy()
            self.homes[i][2]=DefaultHeight
    def CloseLinks(self,):
        #cflib.crtp.init_drivers()
        #factory = CachedCfFactory(ro_cache='./cache',rw_cache='./cache')
        # #SWARM = Swarm(URIS, factory)
